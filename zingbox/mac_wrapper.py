...
def post_auth(authcred, attributes, authret, info):
    ...
    if attributes.get('vpn_auth'):                  # only do this for VPN authentication
        ...
        if hw_addr:
            ...
            if mac_not_in_db(hw_addr_save, hw_addr):
                error = "BLOCK MAC %s" % hw_addr
                authret['status'] = FAIL
                ...
                print "***** POST_AUTH MAC CHECK: connection attempt   : BLOCKED"
            else:
                error = ""
                authret['status'] = 0 
                authret['reason'] = error          # this error string is written to the server log file
                authret['client_reason'] = error   # this error string is reported to the client user
                print "***** POST_AUTH MAC CHECK: connection attempt   : PASSED"
...
def mac_not_in_db(hw_addr_save, hw_addr):
    """
    Find if MAC address does not match known users, then we need to flag/block by sending True.

    :param hw_addr_save:
      previous HW address saved
    :param hw_addr:
    :return: True if addr needs to be blockd, false if addr is friendly
    """
    if is_found(hw_addr_save):
        return False

    return is_missing(hw_addr)

def is_found(addr):
    # run bash wrapper to query mysql db
    cmd = "/home/openvpnas/docker_mac.sh %s | grep %s 2>/dev/null" % (str(addr), str(addr))

    found = os.popen(cmd).readlines(4096)
    found = " ".join(found).replace("\n", "")
    status =  str(addr) in found
    return status

def is_missing(addr):
    return not is_found(addr)

# eventually should use mysqldb but got blocked (access denied) below so using popen instead
#def is_found(addr):
#    db = MySQLdb.connect(host="127.0.0.1",  # your host 
#                         user="readonly",       # username
#                         port=6603,         # docker compose port
#                         passwd='',
#                         db="vpndb")   # name of the database
